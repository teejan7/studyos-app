...
export default function StudyHeatmap({ entries }: StudyHeatmapProps) {
  return (
    <div className="rounded-xl border border-border bg-deep p-4 text-sm">
      ...
      <div className="mt-4 grid gap-1 text-center text-xs text-text" style={{ gridTemplateColumns: 'repeat(7, minmax(0, 1fr))' }}>
        {weekDays.map((label) => (
          <div key={label} className="py-1">{label}</div>
        ))}
        {entries.map((entry) => {
          const color = entry.hours >= 5 ? 'bg-accent' : entry.hours >= 3 ? 'bg-dim' : entry.hours >= 1 ? 'bg-muted' : 'bg-border';
