...
export default function QuestionFilter({ subjects, filters, onChange }: QuestionFilterProps) {
  return (
    <div className="grid gap-3 lg:grid-cols-[1fr_1fr_1fr_auto]">
      <select value={filters.subject} onChange={(event) => onChange({ subject: event.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm text-green-100">
        <option value="">All subjects</option>
        {subjects.map((subject) => (
          <option key={subject.code} value={subject.code}>{subject.name}</option>
        ))}
      </select>
      <select value={filters.module} onChange={(event) => onChange({ module: event.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm text-green-100">
        <option value="">All modules</option>
        {[1, 2, 3, 4, 5].map((module) => (
          <option key={module} value={String(module)}>Module {module}</option>
        ))}
      </select>
      <select value={filters.type} onChange={(event) => onChange({ type: event.target.value })} className="rounded-lg border border-border bg-bg px-3 py-2 text-sm text-green-100">
      ...
      <label className="inline-flex items-center rounded-lg border border-border bg-bg px-3 py-2 text-sm text-green-100">
